...
void (*turn_callback_)() = NULL;

void complete_turn(int result) {
  if (result) {
	if (dbugflg)
		console_printf("loc = %d, verb = %d, object = %d, \
...
void turn(void (*callback)())
{
  turn_callback_ = callback;
	char	i;
	/*
...
/*	causes occasional "move" with no describe & descitem	*/
  ...
  console_printf("Getting player input\n");

  english(complete_turn);
}
...
void dotrav()
{
	...
	for (kk = 0; travel[kk].tdest>=0 && !mvflag; ++kk) {
		...
		if (dbugflg)
			console_printf("rdest = %d, rverb = %d, rcond = %d, \
			                robject = %d in dotrav\n", rdest, rverb, \
              			rcond, robject);
	...
	if (!mvflag)
		...
		newloc = rdest;
		if (dbugflg)
			console_printf("newloc in dotrav = %d\n", newloc);
	}
}
...
{
	if (!closing) {
		yes(81+numdie*2, 82+numdie*2, 54, continue_death);
		return;
	}
	/*
...
void trobj()
{
	if (verb)
		...
		screen_printf("What do you want to do with the %s?\n",
			probj(object));
}
...
void dwarves()
{
	...
	*/
	...
	if (dtotal == 0)
		return;
	if (dtotal > 1)
		screen_printf("There are %d threatening little dwarves in the room with you!\n", dtotal);
	else
		rspeak(4);
	if (attack == 0)
		return;
	if (dflag == 2)
		++dflag;
	if (attack > 1) {
		screen_printf("%d of them throw knives at you!!\n", attack);
		k = 6;
	}
	else {
		rspeak(5);
	...
	if (stick <= 1) {
		...
		screen_printf("%d of them get you !!!\n", stick);
	oldloc2 = newloc;
	death();
}
/*
