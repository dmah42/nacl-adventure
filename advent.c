...
void continue_main(int do_instructions) {
  if (do_instructions)
    limit = 1000;
  else
    limit = 330;
...
void nacl_main() {
  ...
  initplay();
  console_printf("Asking the user about instructions..\n");
  yes(65, 1, 0, continue_main);
}
#endif
...
void opentxt()
{
	fd1 = fopen(ADV1, "r");
	if (!fd1) {
		console_printf("Sorry, I can't open advent1.txt...\n");
		exit(-1);
	}
	fd2 = fopen(ADV2, "r");
	if (!fd2) {
		console_printf("Sorry, I can't open advent2.txt...\n");
		exit(-1);
	}
	fd3 = fopen(ADV3, "r");
	if (!fd3) {
		console_printf("Sorry, I can't open advent3.txt...\n");
		exit(-1);
	}
	fd4 = fopen(ADV4, "r");
	if (!fd4) {
		console_printf("Sorry, I can't open advent4.txt...\n");
		exit(-1);
	}
}
#endif
...
void saveadv()
{
	...
	if (savefd == NULL) {
		console_printf("Sorry, I can't save your game...\nAborting program...\n");
		exit(-1);
	}
	...
	if (fclose(savefd)  ==  -1) {
		console_printf("Sorry, I can't close the file...%s\n", \
		       username);
		exit(-1);
	}
	screen_printf("Game saved -- see you later...\n");
}
...
void restore()
{
  ...
  game_name(username);
	...
	if (restfd == NULL) {
		console_printf("Sorry, no game to load...\n");
		return;
	}
	...
	if (fclose(restfd)  ==  -1) {
		console_printf("Warning -- can't close save file...%s\n", \
		       username);
	}
	screen_printf("Game restored...\n");
	describe();
}
