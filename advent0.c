...
int Ltoa(int a, char* str) {
  sprintf(str, "%d", a);
  return strlen(str);
}

int main(int argc, char** argv) {
	FILE* isam = fopen("advtext.h", "w");
	if (!isam) {
		printf("Sorry, I can't open advtext.h...\n");
		exit(-1);
	}
	FILE* fd1 = fopen("advent1.txt", "r");
	if (!fd1) {
		printf("Sorry, I can't open advent1.txt...\n");
		exit(-1);
	}
	FILE* fd2 = fopen("advent2.txt", "r");
	if (!fd2) {
		printf("Sorry, I can't open advent2.txt...\n");
		exit(-1);
	}
	FILE* fd3 = fopen("advent3.txt", "r");
	if (!fd3) {
		printf("Sorry, I can't open advent3.txt...\n");
		exit(-1);
	}
	FILE* fd4 = fopen("advent4.txt", "r");
	if (!fd4) {
